...
<div>
	<div class="collection category-box col s2 left-align">
		<c:forEach var="spaceCategoryDTO" items="${spaceCategoryList }">
			<a href="#!" class="collection-item tab_list2" name="space-category-no_${ spaceCategoryDTO.space_category_no }"> <span class="badge">1</span>${spaceCategoryDTO.space_category_name }
			</a>
		</c:forEach>
	</div>
	...
	<c:forEach var="spaceCategoryDTO" items="${spaceCategoryList }">
		<c:choose>
			<c:when test="${ count eq 0 }">
				<div class="tabDiv2 board-list-box z-depth-1 col s9" style="display: block;">
									...
									<table>
										...
										<tbody>
											<c:forEach var="spaceBoardDTO" items="${ spaceCategoryDTO.spaceBoardList }">
												<c:choose>
													<c:when test="${ spaceBoardDTO.member_no eq sessionScope.member_no }">
														<tr class="hoverable">
			...
			<c:otherwise>
				<div class="tabDiv2 board-list-box z-depth-1 col s9" style="display: none;">
					<span>${ spaceCategoryDTO.space_category_name }</span>
					<c:choose>
						<c:when test="${empty spaceCategoryDTO.spaceBoardList }">
							<div class="null-space-box">
